...
export default function TravellerPage() {
  ...
  const loadStoriesData = async (targetPage: number, isInitial: boolean = false) => {
    if (!travellerId) return;
    if (isInitial) setIsInitialLoading(true);
    setIsLoadingMore(true);

    try {
      const res = await api.get(`/users/${travellerId}/stories?page=${targetPage}&perPage=6`);
      ...
      const adaptedStories = rawArticles.map((article: any) => {
        ...
        return {
          ...
          rating: typeof article.rate === "number" ? article.rate : (article.rating || 0), 
      ...
      if (isInitial) {
        ...
        setHasMore(adaptedStories.length > 0 && adaptedStories.length < totalItems);
      } else {
...
{/* TravellerInfo буде працювати після виправлення коду на бекенді. Отримати юзера за айді - роут: users/userId: не працює на бекенді */}
      ...
      <TravellerInfo name="Test" avatarUrl="test" storiesCount={5} />
      </section>
      ...
      {isInitialLoading ? (
        <div style={{ textAlign: "center", margin: "40px 0" }}><Loader /></div>
      ) : stories.length === 0 ? (
